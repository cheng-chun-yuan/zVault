 ...
 */
export function parseNameEntry(data: Uint8Array): NameEntry | null {
  if (data.length < 179) {
    return null;
  }
  ...
  const createdAt = new Date(
    Number(new DataView(createdAtBytes.buffer).getBigInt64(0, true)) * 1000
  );
  const updatedAt = new Date(
    Number(new DataView(updatedAtBytes.buffer).getBigInt64(0, true)) * 1000
  );
 ...
 */
export function entryToStealthAddress(entry: NameEntry): StealthMetaAddress {
  return {
