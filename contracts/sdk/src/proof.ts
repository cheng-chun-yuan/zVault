 ...
 */
export function getNoirCircuitsDir(): string {
  ...
  throw new Error(
    "Noir circuits directory not found. Make sure noir-circuits is available."
  );
}
 ...
 */
export function isNargoAvailable(): boolean {
  try {
 ...
 */
export function isProofGenerationAvailable(): boolean {
 ...
 */
function toProverToml(inputs: Record<string, string | string[]>): string {
  const lines: string[] = [];
...
export async function generateProof(
  ...
  if (!isProofGenerationAvailable()) {
    throw new Error(
      "Proof generation not available. Install nargo and run 'bun install' in noir-circuits/"
    );
  }
  ...
  execSync(
    `"${bbPath}" prove -b "${bytecode}" -w "${witness}" -o "${proofOutputDir}" --write_vk`,
...
export async function verifyProof(
  ...
  try {
    execSync(
      `"${bbPath}" verify -p "${join(proofsDir, "proof")}" -k "${join(proofsDir, "vk")}"`,
      { cwd: circuitPath, stdio: "pipe" }
    );
    return true;
  } catch {
...
export async function generateClaimProof(
  ...
  const inputs = {
    ...
    merkle_path: merkleProof.pathElements.map(
...
export async function generateTransferProof(
  ...
  const inputs = {
    ...
    merkle_root: "0x" + Buffer.from(merkleProof.root).toString("hex"),
    nullifier_hash:
      "0x" + inputNote.nullifierHash.toString(16).padStart(64, "0"),
    output_commitment:
      "0x" + outputNote.commitment.toString(16).padStart(64, "0"),
  };
...
export async function generateSplitProof(
  ...
  const inputs = {
    ...
    input_nullifier_hash:
      "0x" + inputNote.nullifierHash.toString(16).padStart(64, "0"),
    output_commitment1:
      "0x" + output1Note.commitment.toString(16).padStart(64, "0"),
    output_commitment2:
      "0x" + output2Note.commitment.toString(16).padStart(64, "0"),
  };
...
export async function generatePartialWithdrawProof(
  ...
  const inputs = {
    ...
    change_commitment:
      "0x" + changeNote.commitment.toString(16).padStart(64, "0"),
 ...
 */
export function serializeProof(proof: NoirProof): Uint8Array {
  // Format: proof_len (4) | proof | vk_len (4) | vk | vk_hash (32) | public_inputs_count (4) | public_inputs
  const encoder = new TextEncoder();
  const publicInputsEncoded = proof.publicInputs.map((pi) =>
    encoder.encode(pi + "\n")
  );
  const publicInputsLen = publicInputsEncoded.reduce(
 ...
 */
export function deserializeProof(data: Uint8Array): NoirProof {
