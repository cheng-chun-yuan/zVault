...
//!
//! Provides transaction parsing and hash computation for Bitcoin transactions.

use pinocchio::program_error::ProgramError;

use crate::shared::crypto::sha256::double_sha256;
...
mod tests {
    ...
    fn test_compute_txid() {
        // Simple test - txid should be reversed hash
        let raw_tx = [0u8; 100];
        let hash = compute_tx_hash(&raw_tx);
        let txid = compute_txid(&raw_tx);

        let mut expected_txid = hash;
