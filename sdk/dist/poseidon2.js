...
export const deriveNotePubKey = (sharedX, sharedY) => zkPoseidon2([sharedX, sharedY]);
/** Compute commitment for Merkle tree */
export const computeCommitment = (notePubKey, amount, random = 0n) => zkPoseidon2([notePubKey, amount, random]);
/** Compute nullifier from spending key and leaf index */
export const computeNullifier = (spendingPriv, leafIndex) => zkPoseidon2([spendingPriv, leafIndex]);
/** Hash nullifier for double-spend prevention */
export const hashNullifier = (nullifier) => zkPoseidon2([nullifier]);
/** Compute note from nullifier and secret (legacy) */
export const computeNote = (nullifier, secret) => zkPoseidon2([nullifier, secret]);
// Legacy helpers
/** @deprecated Use computeCommitment */
export const computeCommitmentLegacy = (nullifier, secret, amount) => zkPoseidon2([zkPoseidon2([nullifier, secret]), amount]);
/** @deprecated Use computeNullifier */
export const computeNullifierHashLegacy = (nullifier) => zkPoseidon2([nullifier]);
// No-op for backwards compatibility
export const initPoseidon2Sync = async () => { };
