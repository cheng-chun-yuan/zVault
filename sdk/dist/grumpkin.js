...
export const GRUMPKIN_FIELD_PRIME = 21888242871839275222246405745257275088548364400416034343698204186575808495617n;
 ...
 */
export const GRUMPKIN_INFINITY = { x: 0n, y: 0n };
/**
 ...
 */
export function pointNegate(point) {
    if (isInfinity(point))
        return point;
    return {
        x: point.x,
 ...
 */
export function pointToCompressedBytes(point) {
    if (isInfinity(point)) {
 ...
 */
export function pointFromCompressedBytes(bytes) {
    if (bytes.length !== 33) {
 ...
 */
export function ecdhSharedSecret(privKey, pubKey) {
 ...
 */
export function pubKeyToBytes(pubKey) {
    return pointToCompressedBytes(pubKey);
}
/**
 * Convert bytes back to public key
 */
export function pubKeyFromBytes(bytes) {
    if (bytes.length === 33) {
