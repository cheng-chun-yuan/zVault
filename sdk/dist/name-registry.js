 ...
 */
export function deriveNameRegistryPDA(name, programId = ZVAULT_PROGRAM_ID) {
 ...
 */
export function parseNameRegistry(data, name) {
 ...
 */
export async function lookupZkeyName(connection, name, programId = ZVAULT_PROGRAM_ID) {
    const normalized = normalizeName(name);
    ...
    try {
        ...
        const [pda] = PublicKey.findProgramAddressSync([Buffer.from(NAME_REGISTRY_SEED), Buffer.from(nameHash)], new PublicKey(programId));
 ...
 * @returns Stealth address or null if not found
 */
export async function lookupZkeyNameWithPDA(getAccountInfo, name) {
    const normalized = normalizeName(name);
    try {
