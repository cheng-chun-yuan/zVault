...
// =============================================================================
// Stealth Deposit V2 Types (Backend-Managed)
// =============================================================================

/// Status of a stealth deposit through its lifecycle
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum StealthDepositStatus {
    /// Waiting for BTC deposit to arrive
    Pending,
    /// BTC transaction detected in mempool
    Detected,
    /// Waiting for deposit confirmations
    Confirming,
    /// Deposit confirmed, ready to sweep
    Confirmed,
    /// Sweeping UTXO to vault
    Sweeping,
...
/// A stealth deposit record tracking the backend-managed deposit flow
///
/// Flow:
/// 1. User provides viewing + spending pubkeys
/// 2. Backend generates ephemeral keypair, derives BTC address
/// 3. User sends BTC to that address
/// 4. Backend detects, sweeps, verifies on-chain
/// 5. User scans stealth inbox, finds their deposit
#[derive(Debug, Clone, Serialize, Deserialize)]
