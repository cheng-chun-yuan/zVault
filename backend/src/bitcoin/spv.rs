...
impl SpvProofGenerator {
    ...
    pub async fn get_block_header_by_hash(
        &self,
        block_hash: &str,
    ) -> Result<(BlockHeader, u64), SpvError> {
    ...
    pub async fn generate_proof(&self, txid: &str, vout: u32) -> Result<SpvProof, SpvError> {
        ...
        // Check if confirmed
        let status = tx_info.status.as_ref().ok_or(SpvError::TxNotConfirmed)?;

        if !status.confirmed {
            return Err(SpvError::TxNotConfirmed);
        }

        let block_hash = status.block_hash.as_ref().ok_or(SpvError::TxNotConfirmed)?;
        let block_height = status.block_height.ok_or(SpvError::TxNotConfirmed)?;
